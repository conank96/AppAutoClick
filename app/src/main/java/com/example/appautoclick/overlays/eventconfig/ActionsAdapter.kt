 ...
 */
package com.example.appautoclick.overlays.eventconfig
...
import com.example.appautoclick.overlays.utils.getIconRes
import com.example.appautoclick.R
import com.example.appautoclick.databinding.ItemActionCardBinding
import com.example.appautoclick.databinding.ItemNewCopyCardBinding
...
class ActionViewHolder(private val viewBinding: ItemActionCardBinding) : RecyclerView.ViewHolder(viewBinding.root) {
    ...
    fun onBind(action: Action, actionClickedListener: (Int, Action) -> Unit) {
        ...
        itemView.setOnClickListener { actionClickedListener.invoke(adapterPosition, action) }
    }
}
...
class ActionReorderTouchHelper : ItemTouchHelper.SimpleCallback(0, 0) {
    ...
    override fun getMovementFlags(recyclerView: RecyclerView, viewHolder: RecyclerView.ViewHolder): Int {
        val dragFlags = if (viewHolder.adapterPosition == recyclerView.adapter!!.itemCount - 1) 0
