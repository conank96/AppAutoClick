...
import com.example.appautoclick.database.bitmap.BitmapManager
...
import com.example.appautoclick.database.Repository
import com.example.appautoclick.database.domain.*
import com.example.appautoclick.database.domain.toAction
import com.example.appautoclick.database.domain.toCondition
import com.example.appautoclick.database.domain.toEvent
...
internal class RepositoryImpl internal constructor(
    ...
    override fun getEventList(scenarioId: Long): Flow<List<com.example.appautoclick.database.domain.Event>> =
        eventDao.getEvents(scenarioId).mapList { it.toEvent() }

    override fun getCompleteEventList(scenarioId: Long): Flow<List<com.example.appautoclick.database.domain.Event>> =
    ...
    override fun getAllEvents(): Flow<List<com.example.appautoclick.database.domain.Event>> = eventDao.getAllEvents().mapList { it.toEvent() }
    ...
    override suspend fun addEvent(event: com.example.appautoclick.database.domain.Event): Boolean {
    ...
    override suspend fun updateEvent(event: com.example.appautoclick.database.domain.Event) {
        event.conditions?.let {
            saveNewConditionsBitmap(it)
        }
    ...
    override suspend fun updateEventsPriority(events: List<com.example.appautoclick.database.domain.Event>) {
        eventDao.updateEventList(events.map { it.toEntity() })
    }

    override suspend fun removeEvent(event: com.example.appautoclick.database.domain.Event) {
